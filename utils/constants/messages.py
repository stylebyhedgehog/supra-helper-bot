...
class PPM_ATTENDANCE:
    # Ошибка в случае отсутсвия в системе детей, привязанных к родителю
    ERROR_CHILDREN_NOT_FOUND = "Ошибка системы! Не удалось получить информацию об учениках."

    # Ошибка в случае отсутсвия в alfa.crm групп, привязанных к ребенку
    ERROR_GROUPS_NOT_FOUND = "Ошибка системы! Не удалось получить информацию о группах."

    # Ошибка в случае отсутсвия в alfa.crm занятий, проведенных за выбранный месяц
    ERROR_LESSONS_NOT_FOUND = "Не удалось получить информацию о занятиях за выбранный Вами период."

    # Информационное сообщение, к которому прикрепляется кнопки с именами детей (если у родителя  >1 ребенка)
    INFO_CHILD_SELECTION = "Выберите ребенка, у которого хотите проверить посещаемость:"

    # Информационное сообщение, к которому прикрепляются кнопки с названиями групп (кнопки отображаются только если ребенок состои >1 группе)
    INFO_GROUP_SELECTION = "Выберите группу, относительно которой хотите проверить посещаемость:"

    # Информационное сообщение, к которому прикрепляются кнопки с месяцами
...
# Сообщения бота при выполнении активности "Узнать баланс"
class PPM_PERFORMANCE:
    # Ошибка в случае отсутсвия в системе детей, привязанных к родителю
    ERROR_CHILDREN_NOT_FOUND = "Ошибка системы! Не удалось получить информацию об учениках."

    # Ошибка в случае отсутсвия в alfa_requests.crm групп, привязанных к ребенку
    ERROR_GROUPS_NOT_FOUND = "Ошибка системы! Не удалось получить информацию о группах."
    ...
    # Информационное сообщение, к которому прикрепляется кнопки с именами детей (если у родителя  >1 ребенка)
    INFO_CHILD_SELECTION = "Выберите ребенка, у которого хотите проверить успеваемость и посещаемость:"

    # Информационное сообщение, к которому прикрепляются кнопки с названиями групп (кнопки отображаются только если ребенок состои >1 группе)
    INFO_GROUP_SELECTION = "Выберите группу, относительно которой хотите проверить успеваемость и посещаемость:"

    # Информационное сообщение, к которому прикрепляются кнопки с месяцами
    INFO_MONTH_SELECTION = "Выберите месяц, за который хотите проверить успеваемость:"
