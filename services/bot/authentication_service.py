...
class AuthenticationService:
    ...
    def is_admin_authorized(admin_telegram_id):
        admin_in_system = AdministratorRepository.find_administrator_by_telegram_id(admin_telegram_id)
        if admin_in_system:
            return True
        return False
    ...
    def is_parent_authorized(parent_telegram_id):
        parent_in_system = ParentRepository.find_parent_by_telegram_id(parent_telegram_id)
        if parent_in_system:
            return True
        return False
    ...
    def authorize_admin(password, admin_telegram_id, username):
        if os.getenv("ADMIN_TG_BOT_PASSWORD") in password:
            AdministratorRepository.save_administrator(admin_telegram_id, username)
            return username
        else:
            return None
    ...
    def authorize_parent(phone_number, parent_telegram_id):
        ...
        if parent_children:
            ...
            ParentRepository.save_parent(parent_telegram_id, parent_name, normalized_phone_number)
            for parent_child in parent_children:
                child_id = parent_child.get("child_id")
                child_name = parent_child.get("child_name")
                ChildRepository.save_child(parent_telegram_id, child_id, child_name)
